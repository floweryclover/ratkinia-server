...
T_PlayerCharacters::T_PlayerCharacters(connection& connection)
    : Table{ connection }
{
         ...
         const auto row : playerCharacters)
    {
        ...
        playerCharactersByPlayerId_[pair->second.PlayerId].push_back(pair->second.Id);
        playerCharactersByName_[pair->second.Name] = pair->second.Id;
    }
}

// std::pair<T_PlayerCharacters::CreateCharacterResult, uint64_t> T_PlayerCharacters::TryCreatePlayerCharacter(
